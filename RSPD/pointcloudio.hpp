...
public:
    ...
    void savePlanesAsXYZ(const PointCloud3d *pointCloud, const Geometry *geometry,  const std::string &output_folder)
    {
        size_t numPlanes = geometry->numPlanes();
        for (size_t i = 0; i < numPlanes; i++)
        {
            std::ofstream oFile(output_folder + "plane_" + std::to_string(i) + ".txt");

            const Plane *plane = geometry->plane(i);
            const std::vector<size_t> &inliers = plane->inliers();
            for(size_t inlier:inliers){
                oFile << pointCloud->at(inlier).position().data()[0] << " " << pointCloud->at(inlier).position().data()[1]  << " " << pointCloud->at(inlier).position().data()[2] << std::endl;
